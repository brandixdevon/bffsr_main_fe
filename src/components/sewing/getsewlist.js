...
const headCells = [
  ...
  { id: 'sew_status', numeric: true, disablePadding: false, label: 'SEWING STATUS', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const [SRFID, setSRFID] = React.useState(window.location.href.split('/').reverse()[0]);
  ...
  const TextFieldChange = (e) => 
  {
    ...
    {
      window.location = '/SEWLIST/' + e.target.value;
      return;
    }
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/sewing/pendingsewing`)
                .then(res => res.json())
                .then(response => {
  ...
  useEffect(() => {
    fetch(`http://${apiurl}/sewing/pendingsewing/${SRFID}`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.sewing);
      })
    ...
    <div className={classes.root}>
      ...
      <div style={{width:"50%"}}>
          <Typography className={classes.title} variant="h6" id="tableTitle" >
            All Pending Sewing / Semi-Sewing List
          </Typography>
      </div>
                    ...
                    <TableCell align="left">
                        ...
                        <Tooltip title="Update Details">
                          <IconButton aria-label="edit" component={ Link } 
                              to={"/SEWIN/" + row.sid_cluster+row.sid_date+row.sid_seq}
                              variant="contained"
                              color="default"
                              size="small">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.sew_status}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
