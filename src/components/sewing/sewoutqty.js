...
export default function Editsrf() {
    ...
    const [VAL_SEW_OUTQTY, setVAL_SEW_OUTQTY] = React.useState([]);
    const [VAL_SEW_OUTCOMMENT, setVAL_SEW_OUTCOMMENT] = React.useState([]);
     
    const OnChange_outqty = (e) => {
      setVAL_SEW_OUTQTY(e.target.value);
    };

    const OnChange_outcomment = (e) => { 
      setVAL_SEW_OUTCOMMENT(e.target.value);
    };
    ...
    const updatedata = (e) => {
        ...
        {
            Swal.fire({  title: 'Error!',  text: 'Sorry! Sewing In Quantity Can not Be Zero or Null.',  icon: 'error',  confirmButtonText: 'OK'});
            return;
        }

        if(VAL_SEW_OUTQTY === null || VAL_SEW_OUTQTY.length <= 0)
        {
            Swal.fire({  title: 'Error!',  text: 'Sorry! Please Enter SEWING OUT Qty.',  icon: 'error',  confirmButtonText: 'OK'});
            return;
        }

        const Reg_Sizeqty = /^[0-9\b]+$/;

        if (!Reg_Sizeqty.test(VAL_SEW_OUTQTY)) 
        {
            Swal.fire({  title: 'Error!',  text: 'Please Enter Integer Value For Sewing Out Qty',  icon: 'error',  confirmButtonText: 'OK'});
            return;
         }

         if(parseInt(VAL_SEW_INQTY) < parseInt(VAL_SEW_OUTQTY))
         {
            Swal.fire({  title: 'Error!',  text: 'Sewing Out Qty Can not be greater than Sewing In Qty.',  icon: 'error',  confirmButtonText: 'OK'});
            return;
         }

        if(VAL_SEW_INQTY !== VAL_SEW_OUTQTY)
        {
          if(VAL_SEW_OUTCOMMENT === null || VAL_SEW_OUTCOMMENT.length <= 0)
          {
        ...
        const sendOptions = {
            ...
            body: JSON.stringify({
                ...
                "outqty": VAL_SEW_OUTQTY,
                "comment": VAL_SEW_OUTCOMMENT,
            })
        };
  
        fetch(`http://${apiurl}/sewing/updatesewoutqty`,sendOptions)
    ...
    const gettoupdate = item =>
    {
        ...
        fetch(`http://${apiurl}/sewing/getsewqtydetails/${item}`)
              ...
              .then(response => {
                ...
                setVAL_SEW_INQTY(response.sewingqtydetails[0].sew_in);
                setVAL_SEW_OUTQTY("");
                setVAL_SEW_OUTCOMMENT("");
                })
              .catch(error => console.log(error));
    ...
    <Grid style={{backgroundColor: 'white'}}  container spacing={1}>
        ...
        <Grid item xs={12} md={12}>
            ...
            <TableHead>
                ...
                <TableCell size="small" style={{"font-weight":800,width:"15%"}} align="right">
                <Typography variant="subtitle2" gutterBottom>Sewing In Qty</Typography>
                <TextField id="txtUnit"  value={VAL_SEW_INQTY} InputProps={{readOnly:true}} variant="outlined" style={{width:"100%"}} size="small"/>
                </TableCell>
                <TableCell size="small" style={{"font-weight":800,width:"15%"}} align="right">
                <Typography variant="subtitle2" gutterBottom>Sewing Out Qty</Typography>
                <TextField id="txtPerimeter"  value={VAL_SEW_OUTQTY} onChange={OnChange_outqty} variant="outlined" style={{width:"100%"}} size="small"/>
                </TableCell>
                <TableCell size="small" style={{"font-weight":800,width:"40%"}} align="right">
                <Typography variant="subtitle2" gutterBottom>Sewing Out Comment</Typography>
                <TextField id="txtPcs"  value={VAL_SEW_OUTCOMMENT} onChange={OnChange_outcomment} variant="outlined" style={{width:"100%"}} size="small"/>
                </TableCell>
            ...
            {DSET_SEWQTY.map((row) => (
              <TableRow key={row.sewqty_id}>
                ...
                <TableCell size="small" align="right">{row.sew_in}<br/><p style={{color:'purple',fontSize:12}}>{row.sew_in_cmt}</p></TableCell>
                <TableCell size="small" align="right">{row.sew_out}</TableCell>
                <TableCell size="small" align="right">{row.sew_out_cmt}</TableCell>
                <TableCell size="small" align="right">
                   { row.sew_out === null || row.sew_out.length <= 0 ?
