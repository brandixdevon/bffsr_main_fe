...
const GetData = () => {
  ...
  const sendOptions = {
    ...
    fetch(`http://${apiurl}/reports/otdhitdata`,sendOptions)
    .then(response => response.json())
    .then(data => 
    {
        if(data.Type === "SUCCESS")
        {
          setDS_DATA(data.otdhit);
          setDOWNLOADDATA(data.otdhit);
        }
        ...
        <Container maxWidth="false" className={classes.container}>
          <Grid container spacing={1}>
            ...
            <Grid item xs={12} md={12}>
                <Typography style={{fontSize:15,color:"purple",fontWeight:700,padding:1}} gutterBottom>SAMPLE ROOM OTD HIT SRF DETAILS</Typography>
            </Grid>
            ...
            <ExcelFile filename={"DELIVERY_FAILURE_DETAILS_"+ new Date().toISOString().slice(0,20)} fileExtension="xlsx" element={<Button variant="contained" style={{color:"white",backgroundColor:"green"}} >Download Excel File</Button>}>
                <ExcelSheet data={DOWNLOADDATA} name="SAMPLE_DELIVERY_DETAILS">
                    ...
                    <ExcelColumn label="DISPATCH DATE" value="dis_start"/>
                </ExcelSheet>
            </ExcelFile>
          ...
          <Grid container spacing={1}>
            ...
            <TableHead>
                <TableRow TableRow>
                    ...
                    <TableCell size="small" style={{width:"15%"}} align="left">
                        <Typography variant="subtitle2" style={{"font-weight":"bold"}} gutterBottom>DISPATCH DATE</Typography>
                    </TableCell>
                </TableRow>
            </TableHead>
            <TableBody>
