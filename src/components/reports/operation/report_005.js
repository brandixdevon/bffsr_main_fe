...
const GetData = () => {
  ...
  const sendOptions = {
    ...
    fetch(`http://${apiurl}/reports/recutdetailsbydate`,sendOptions)
    .then(response => response.json())
    .then(data => 
    {
        if(data.Type === "SUCCESS")
        {
          setDS_DATA(data.recutdata);
          setDOWNLOADDATA(data.recutdata);
        }
        ...
        <Container maxWidth="false" className={classes.container}>
          <Grid container spacing={1}>
            ...
            <Grid item xs={12} md={12}>
                <Typography style={{fontSize:15,color:"purple",fontWeight:700,padding:1}} gutterBottom>SAMPLE ROOM RE-CUTTING OPERATION DETAILS FOR INCENTIVE CALCULATION </Typography>
            </Grid>
            ...
            <ExcelFile filename={"RE_CUTTING_DETAILS_"+ new Date().toISOString().slice(0,20)} fileExtension="xlsx" element={<Button variant="contained" style={{color:"white",backgroundColor:"green"}} >Download Excel File</Button>}>
                <ExcelSheet data={DOWNLOADDATA} name="CUTTING_TEAM_DETAILS">
                    <ExcelColumn label="SRF DOC ID" value="srfdoc"/>
                    <ExcelColumn label="TEAM" value="recut_team"/>
                    <ExcelColumn label="HANDLING" value="recut_handling"/>
                    <ExcelColumn label="METHOD" value="recut_method"/>
