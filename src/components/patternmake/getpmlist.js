...
const headCells = [
  ...
  { id: 'pm_status', numeric: true, disablePadding: false, label: 'PATTERN MAKING STATUS', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/patternmaking/pmpendinglist/${Userid}`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.pendingpatterns);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/patternmaking/pendingpmfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/patternmaking/pmpendinglist/${Userid}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.pendingpatterns);
                })
    ...
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          <Typography className={classes.title} variant="h6" id="tableTitle" >
            All Pattern Making Pending SRF List
          </Typography>
      </div>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <TableCell align="left">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.pm_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
