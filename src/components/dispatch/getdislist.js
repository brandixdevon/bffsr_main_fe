...
const headCells = [
  ...
  { id: 'username', numeric: true, disablePadding: false, label: 'SAMPLE OWNER', width:'10%' },
  ...
  { id: 'dis_status', numeric: true, disablePadding: false, label: 'DISPATCH STATUS', width:'10%' }
];
...
export default function EnhancedTable() {
  ...
  const [DTROWS, setDTROWS] = React.useState([]);
  ...
  const [searchbyuname, setsearchbyuname] = React.useState(false);

    const requestSearch = (searchedVal) => {
      const filteredRows = DTROWS.filter((row) => {
    ...
    const oncheckedchange = () => {
        if(searchbyuname === true)
        {
          setsearchbyuname(false);
        }

        if(searchbyuname === false)
        {
          setsearchbyuname(true);
        }
      };
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/dispatch/pendingdis/${Userid}`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.dispatch);
        setDTROWS(response.dispatch);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/dispatch/pendingdislistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/dispatch/pendingdis/${Userid}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.dispatch);
                  setDTROWS(response.dispatch);
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"45%"}}>
          ...
          All Pending SRF List For Dispatch
          </Typography>
      </div>
      <div style={{width:"20%",textAlign:"right"}}>
          
          <p>Search By User : <Switch onChange={() => oncheckedchange()}/></p>
         
      </div>
      <div hidden={!searchbyuname} style={{width:"35%",textAlign:"right"}}>
          
          <SearchBar value={searched} onChange={(searchVal) => requestSearch(searchVal)} onCancelSearch={() => cancelSearch()} />
         
      </div>
      
      <div hidden={searchbyuname} style={{width:"30%",textAlign:"right"}}>
      ...
      <div hidden={searchbyuname} style={{width:"5%",textAlign:"right"}}>
                    ...
                    <TableCell align="left">
                        ...
                        <Tooltip title="Update Details">
                          <IconButton aria-label="edit" component={ Link } 
                              to={"/dispatchupdate/" + row.srf_id + "/" + row.proelmid}
                              variant="contained"
                              color="default"
                              size="small">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.username}</TableCell>
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.dis_status}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
