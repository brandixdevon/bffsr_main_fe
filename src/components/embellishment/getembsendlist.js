...
const headCells = [
  ...
  { id: 'plant_name', numeric: true, disablePadding: false, label: 'EMB PLANT', width:'15%' },
  { id: 'emb_panel', numeric: true, disablePadding: false, label: 'PANEL', width:'15%' },
...
export default function EnhancedTable() {
  ...
  const [embitemid, setembitemid] = React.useState("");
  ...
  const TextFieldChange = (e) => 
  {
    ...
    {
      window.location = '/ALLEMBSCANLIST/' + e.target.value;
      return;
    }
  ...
  const UpdateRecieve = () => {
    ...
    fetch(`http://${apiurl}/embellishmentscan/updaterecqty`,sendOptions)
  ...
  useEffect(() => {
    ...
    fetch(`http://${apiurl}/embellishmentscan/pendingembscan/${SRFID}`)
    .then(res => res.json())
    .then(response => {
      setDATASET(response.embellishment);
    })
    ...
    if(IsqtyUpdate === true )
    {
        fetch(`http://${apiurl}/embellishmentscan/getpendingcount/${embitemid}`)
        .then(res => res.json())
        .then(response => {
    ...
    if(IsFinished === true)
    {
      ...
      .then(dataNextStep => 
      {
          ...
          { 
            setIsFinished(false);
              Swal.fire({  title: 'Success!',  text: `All Embellishment Items Received for ${recSRF}`,  icon: 'info',  confirmButtonText: 'OK'}).then(function() {
                window.location = "/ALLEMBSCANLIST/ALL";
            });
          }
...
}, [apiurl,SRFID,IsqtyUpdate,IsFinished,recSRF_ID,embitemid,recSRF]);
    ...
    <div className={classes.root}>
      ...
      <div style={{width:"50%"}}>
          <Typography className={classes.title} variant="h6" id="tableTitle" >
            All Embellishment Receive Pending Panels List
          </Typography>
      </div>
                    ...
                    <Tooltip title="View Details">
                        ...
                        <Tooltip title="Scan Receiving">
                          <IconButton aria-label="edit" component={ Link } 
                              onClick={() => handleClickOpenrec(row.embitem_id,row.srfidseq,row.gatepass_no,row.plant_name,row.emb_panel,row.size_name,row.send_qty,row.srf_id)}
                              variant="contained"
                              color="default"
                              size="small">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.emb_panel}</TableCell>
                  ...
                  (<>
                    <TableCell colSpan={8}>SCAN ID : {SRFID}</TableCell>
                        ...
                        <Dialog maxWidth="md" fullWidth={true} onClose={handleCloserec} aria-labelledby="customized-dialog-title-rec" open={openrec} >
                            ...
                            {"Embellishment Receiving Update " }
                            </DialogTitle>
                            <DialogContent dividers>
                            ...
                            <DialogActions>
                            <Button style={{margin:"5px"}} variant="contained" color="primary" onClick={UpdateRecieve}>Mark as Received and SAVE</Button>
                            </DialogActions>
                        </Dialog>
