...
const headCells = [
  ...
  { id: 'pl_status', numeric: true, disablePadding: false, label: 'PLAN READY', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/plan/plnpendinglist/${Userid}`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.plans);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/plan/pendingplnlistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/plan/plnpendinglist/${Userid}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.plans);
                })
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          ...
          All Pending SRF List For Update Planning Details
          </Typography>
      </div>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.pl_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
