...
const headCells = [
  ...
  { id: 'cus_name', numeric: true, disablePadding: false, label: 'CUSTOMER', width:'20%' },
  ...
  { id: 'proelmname', numeric: true, disablePadding: false, label: 'SEWING STAGE', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const TextFieldChange = (e) => 
  {
    ...
    {
      window.location = '/SEWPENDINGFORPLAN/' + e.target.value;
      return;
    }
  ...
  useEffect(() => {
    fetch(`http://${apiurl}/packageprocess/pendingprocessforplan/${SRFID}`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.sewing);
        setDOWNLOADDATA(response.sewing);
      })
    ...
    <div className={classes.root}>
      ...
      <div style={{width:"50%"}}>
          <Typography className={classes.title} variant="h6" id="tableTitle" >
            All Ready To Sewing / Semi-Sewing List
          </Typography>
      </div>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <TableCell align="left">
                      ...
                      <TableCell align="left">{row.sam_stage_title}</TableCell>
                      ...
                      <TableCell align="left">{row.proelmname}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
...
<ExcelFile filename={"SRF_PLANING_"+ new Date().toISOString().slice(0,20)} fileExtension="xlsx" element={<Button variant="contained" style={{color:"white",backgroundColor:"green"}} >Download Excel File</Button>}>
                <ExcelSheet data={DOWNLOADDATA} name="SRF_List_Dataset">
                    ...
                    <ExcelColumn label="SIZE COUNT" value="sizecount"/>
                </ExcelSheet>
            </ExcelFile>
