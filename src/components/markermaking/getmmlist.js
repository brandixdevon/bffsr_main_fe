...
const headCells = [
  ...
  { id: 'pm_status', numeric: true, disablePadding: false, label: 'MARKER MAKING STATUS', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/markermaking/mmpendinglist/${Userid}`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.markermaking);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/markermaking/pendingmmlistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/markermaking/mmpendinglist/${Userid}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.markermaking);
                })
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          ...
          All Pending SRF List For Update Marker Making Details
          </Typography>
      </div>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                        ...
                        <IconButton aria-label="print" component={ Link } 
                            to={"/MMSRFPRINT/" + row.srf_id}
                            target="_blank"
                            variant="contained"
                            color="default"
                            size="small">
                            <PictureAsPdfIcon style={{color:"#456eee"}} />
                        </IconButton>
                        </Tooltip>
                        <Tooltip title="Update Details">
                        <IconButton aria-label="edit" component={ Link } 
                            to={"/mmupdate/" + row.srf_id}
                            variant="contained"
                            color="default"
                            size="small">
                        ...
                        <Tooltip title="Hold">
                        <IconButton aria-label="hold" component={ Link } 
                        ...
                        <Tooltip title="User Comments">
                          <IconButton aria-label="comment" component={ Link } 
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.mm_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
