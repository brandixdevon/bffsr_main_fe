...
export default function Editsrf() {
    ...
    const saveascomplete = () => {
      ...
      fetch(`http://${apiurl}/dyeorwash/dowcompleted`,sendOptions)
    ...
    const OnChange_size = (e) => {
        ...
        fetch(`http://${apiurl}/dyeorwash/getsizeqty/${e.value}`)
        .then(res => res.json())
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                const sendOptions = {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({"srfid" : SRFID , "dowitemid" : item})
                };

                fetch(`http://${apiurl}/dyeorwash/deletepanel`,sendOptions)
                .then(response => response.json())
                .then(data => 
                {
                    if(data.Type === "SUCCESS")
                    {
                        fetch(`http://${apiurl}/dyeorwash/dowitemlist/${SRFID}/${PROELMID}`)
                        .then(res => res.json())
                        .then(response => { setDSET_LIST(response.list); })
                        .catch(error => console.log(error));

                        Swal.fire({  title: 'Deleted!',  text: 'Dye or Wash Entry has been deleted!',  icon: 'info',  confirmButtonText: 'OK'});
                    }
    ...
    const addPanels = () => {
        ...
        if(VAL_QTY.length <= 0)
        {
            Swal.fire({  title: 'Error!',  text: 'Please Enter Sending Qty',  icon: 'error',  confirmButtonText: 'OK'});
            return;
        }
        ...
        fetch(`http://${apiurl}/dyeorwash/addpanels`,sendOptions)
            .then(response => response.json())
            .then(data => 
            {
                if(data.Type === "SUCCESS")
  ...
  const updateGatepass = () => {
    ...
    fetch(`http://${apiurl}/dyeorwash/updategp`,sendOptions)
        .then(response => response.json())
        .then(data => 
        {
            if(data.Type === "SUCCESS")
    ...
    useEffect(() => {
          ...
          fetch(`http://${apiurl}/dyeorwash/dowdetails/${SRFID}/${PROELMID}`)
          .then(res => res.json())
          .then(response => { 
              setVAL_GATEPASS(response.dyeorwash[0].gatepass_no);})
          .catch(error => console.log(error));

          fetch(`http://${apiurl}/dyeorwash/srfsizes/${SRFID}`)
          .then(res => res.json())
          .then(response =>  { setDSET_SIZES(response.sizes); })
          .catch(error => console.log(error));

          fetch(`http://${apiurl}/dyeorwash/srfplants/${SRFID}`)
          .then(res => res.json())
    ...
    <Grid style={{backgroundColor: 'white'}}  container spacing={1}>
        ...
        <Grid style={{backgroundColor: 'white'}}  container spacing={1}>
            <Grid item xs={12} md={12}>
                <Typography variant="subtitle1" style={{color:"purple"}} gutterBottom>DYE/WASH SENDING DETAILS</Typography>
            </Grid>
            ...
            <Grid item xs={12} md={12}>
                <Typography variant="subtitle1" style={{color:"purple"}} gutterBottom>Dye/Wash Panel List</Typography>
