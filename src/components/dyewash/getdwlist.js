...
const headCells = [
  ...
  { id: 'emb_status', numeric: true, disablePadding: false, label: 'EMBELLISHMENT STATUS', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/dyeorwash/pendingdow`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.dyewash);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/dyeorwash/pendingdowlistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/dyeorwash/pendingdw`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.dyewash);
                })
  ...
  useEffect(() => {
    fetch(`http://${apiurl}/dyeorwash/pendingdow`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.dyewash);
      })
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          ...
          All Pending SRF List For Dye or Wash Sending
          </Typography>
      </div>
                    ...
                    <TableCell align="left">
                        ...
                        <Tooltip title="Update Details">
                          <IconButton aria-label="edit" component={ Link } 
                              to={"/dowupdate/" + row.srf_id + "/" + row.proelmid}
                              variant="contained"
                              color="default"
                              size="small">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.dow_status}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
