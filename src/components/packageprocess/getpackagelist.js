...
const headCells = [
  ...
  { id: 'cus_name', numeric: true, disablePadding: false, label: 'CUSTOMER', width:'14%' },
  ...
  { id: 'sew_status', numeric: true, disablePadding: false, label: 'SEWING STEP', width:'15%' },
  { id: 'srf_id', numeric: true, disablePadding: false, label: 'DONE', width:'5%' }
];
...
function EnhancedTableHead(props) {
  ...
  return (
    <TableHead>
      <TableRow>
      <TableCell width={"12%"} style={{fontSize:"10px",fontWeight:"bold",color:"#0064b5"}}>#ACTIONS</TableCell>
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/packageprocess/pendingprocess/ALL`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.sewing);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/packageprocess/pendingprocesslistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "comment":COMMENT
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/placementboard/pbpendinglist/${Userid}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.pendingboards);
                })
  ...
  function markasdone(val_srfid,val_elmid)
  {
        if(val_srfid.length <= 0)
        {
            Swal.fire({  title: 'Error!',  text: 'Sorry! Can not Find SRF ID.',  icon: 'error',  confirmButtonText: 'OK'});
            return;
        }

        if(val_elmid.length <= 0)
        {
            Swal.fire({  title: 'Error!',  text: 'Sorry! Can not Find Correct Sewing Entry.',  icon: 'error',  confirmButtonText: 'OK'});
            return;
        }
    
        Swal.fire({
            title: 'Are you sure?',
            text: "Do You Complete this Package.!",
            icon: 'warning',
            ...
            if (result.value) {
              ...
              const sendOptions = {
                ...
                fetch(`http://${apiurl}/packageprocess/updateprocess`,sendOptions)
                .then(response => response.json())
                .then(data => 
                {
                    if(data.Type === "SUCCESS")
                    {
                        Swal.fire({  title: 'Success!',  text: data.Msg,  icon: 'info',  confirmButtonText: 'OK'}).then(function() {
                            window.location = "/PACKAGEPROCESS/";
                        });
                    }
  ...
  useEffect(() => {
    fetch(`http://${apiurl}/packageprocess/pendingprocess/ALL`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.sewing);
      })
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          ...
          All Pending SRF List For Sewing Package Ready
          </Typography>
      </div>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.proelmname.replace(/"/g, "")}</TableCell>
                      <TableCell style={{fontSize:"13px"}} align="left">
                        <Tooltip title="Complete And Send To Sewing">
                          <Button size={"small"} onClick={() => markasdone(row.srf_id,row.proelmid)} style={{color:"white",backgroundColor:"#2E8B57"}} variant="contained">
                            DONE
                          </Button>
                        </Tooltip>
                      </TableCell>
