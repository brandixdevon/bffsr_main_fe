...
const headCells = [
  ...
  { id: 'srf_reqdate', numeric: true, disablePadding: false, label: 'REQUIRE DATE', width:'11%' },
  { id: 'srf_plandate', numeric: true, disablePadding: false, label: 'PLAN DATE', width:'11%' },
  { id: 'graphic', numeric: true, disablePadding: false, label: 'GRAPHIC', width:'6%' },
  { id: 'wash_development', numeric: true, disablePadding: false, label: 'DYE WASH', width:'6%' },
  { id: 'pb_status', numeric: true, disablePadding: false, label: 'PLACEMENT BOARD READY', width:'8%' }
];
    ...
    >
      ...
      {numSelected > 0 ? (
        ...
        <Typography className={classes.title} variant="h6" id="tableTitle" component="div">
          All Pending SRF List For Update Placement Board Ready Details
        </Typography>
      )}
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/placementboard/pbpendinglist/${Userid}`)
      .then(res => res.json())
      .then(response => {
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/placementboard/pendingpbfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                        ...
                        <Tooltip title="Hold">
                        <IconButton aria-label="view" component={ Link } 
                            onClick={() => handleClickOpen(row.srf_id)}
                            variant="contained"
                            color="secondary"
                            size="small">
                            <PanToolIcon />
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.graphic}</TableCell>
                      <TableCell style={{fontSize:"13px"}} align="left">{row.wash_development}</TableCell>
                      <TableCell style={{fontSize:"13px"}} align="left">{row.pb_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
