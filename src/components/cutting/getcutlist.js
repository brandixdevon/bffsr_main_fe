...
const headCells = [
  ...
  { id: 'cut_status', numeric: true, disablePadding: false, label: 'CUTTING STATUS', width:'18%' }
];
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/cutting/cutpendinglist/${Userid}`)
      .then(res => res.json())
      .then(response => {
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/cutting/pendingcutlistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          ...
          All Pending SRF List For Cutting Operations
          </Typography>
      </div>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.cut_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
