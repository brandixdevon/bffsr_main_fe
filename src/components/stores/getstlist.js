...
const headCells = [
  ...
  { id: 'st_status', numeric: true, disablePadding: false, label: 'STORE READY', width:'18%' }
];
...
export default function EnhancedTable() {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/stores/stpendinglist/${SRFID}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.stores);
                })
  ...
  const TextFieldChange = (e) => 
  {
    ...
    {
      window.location = '/STLIST/' + e.target.value;
      return;
    }
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.st_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
