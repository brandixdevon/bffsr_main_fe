...
const headCells = [
  ...
  { id: 'proelmname', numeric: false, disablePadding: false, label: 'QC STEP', width:'10%' },
  { id: 'cut_status', numeric: true, disablePadding: false, label: 'QC STATUS', width:'10%' }
];
...
export default function EnhancedTable() {
  ...
  const FilterSearch = async () => 
  {
    ...
    {
      await fetch(`http://${apiurl}/quality/pendingqc`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.qclist);
      })
    ...
    else
    {
                ...
                await fetch(`http://${apiurl}/quality/pendingqclistfilter`,sendOptions)
                .then(res => res.json())
                .then(response => {
      ...
      body: JSON.stringify({
          ...
          "proelmid":HoldElmID
            ...
            .then(data => 
            {
                ...
                fetch(`http://${apiurl}/cutting/cutpendinglist/${Userid}`)
                .then(res => res.json())
                .then(response => {
                  setDATASET(response.cutting);
                })
  ...
  const handleClosecomment = () => 
  {
   ...
   useEffect(() => {
    fetch(`http://${apiurl}/quality/pendingqc`)
      .then(res => res.json())
      .then(response => {
        setDATASET(response.qclist);
      })
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{width:"65%"}}>
          ...
          All Pending SRF List For Quality Checking
          </Typography>
      </div>
                  ...
                  return (
                    ...
                    <Tooltip title="View Details">
                        ...
                        <IconButton aria-label="edit" component={ Link } 
                            to={"/QCUPDATE/" + row.qc_id + "/" + row.proelmid + "/" +row.srf_id}
                            variant="contained"
                            color="default"
                            size="small">
                      ...
                      <TableCell style={{fontSize:"13px"}} align="left">{row.qc_status.replace(/"/g, "")}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
